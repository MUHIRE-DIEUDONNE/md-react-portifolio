...
import {
  ...
  FiBriefcase, FiCode, FiMail, FiAward,
...
/* ─────────────────────────────────────────────
   ...
   Pass 2: pared back to a quieter, single-ring core, simpler persona
   pills, and a tighter card so the whole widget reads as one calm
   object instead of a dashboard of moving parts.
───────────────────────────────────────────── */
const style = `
  ...
  :root {
    ...
    --au-shadow: 0 30px 80px rgba(2,3,10,0.72), 0 0 0 1px rgba(255,255,255,0.06), 0 0 60px rgba(167,139,250,0.14);
  ...
  .au-aurora span { position: absolute; border-radius: 50%; filter: blur(60px); opacity: 0.28; }
  ...
  .au-aurora span:nth-child(2) { width: 220px; height: 220px; bottom: -50px; right: -30px; background: var(--au-violet); animation: drift-b 17s ease-in-out infinite; }
  .au-aurora span:nth-child(3) { width: 180px; height: 180px; bottom: 30%; left: 8%; background: var(--au-pink); animation: drift-c 20s ease-in-out infinite; opacity: 0.18; }

  @keyframes meter { 0%,100% { height: 30%; opacity: 0.5; } 50% { height: 100%; opacity: 1; } }
  .meter-bar { width: 2px; border-radius: 1px; background: var(--au-cyan); animation: meter 0.85s ease-in-out infinite; transition: background 0.25s; }
  ...
  /* Signature element: a single thin ring, spinning only while active,
     plus a solid core dot. No bar-cluster, no eye-blink — one clean
     gesture that reads instantly as "listening / thinking / speaking". */
  @keyframes core-spin { to { transform: rotate(360deg); } }
  .au-core-ring { transform-origin: 23px 23px; }
  .au-core-ring.is-active { animation: core-spin 3.2s linear infinite; }
  .au-core-ring.is-speaking { animation-duration: 1.6s; }

  @keyframes core-pulse { 0%,100% { opacity: 0.55; transform: scale(1); } 50% { opacity: 1; transform: scale(1.08); } }
  .au-core-dot.is-active { animation: core-pulse 1.1s ease-in-out infinite; }
  ...
  @keyframes typedot { 0%,80%,100% { transform: translateY(0); opacity: 0.35; } 40% { transform: translateY(-3px); opacity: 1; } }
  ...
  .au-mic-btn.is-listening { box-shadow: 0 0 18px rgba(244,114,182,0.4), 0 0 0 4px rgba(244,114,182,0.12); }

  .au-chip { transition: background 0.18s, border-color 0.18s, transform 0.18s, color 0.18s; touch-action: manipulation; }
  .au-chip:hover { background: rgba(167,139,250,0.12) !important; border-color: var(--au-border-bright) !important; }

  .au-persona-btn { transition: background 0.18s ease, border-color 0.18s ease, color 0.18s ease, transform 0.18s ease; touch-action: manipulation; }
  .au-persona-btn.is-active { transform: scale(1.02); }

  .au-bubble { transition: transform 0.18s ease; }
  ...
  .au-toolbtn { transition: background 0.18s, color 0.18s, border-color 0.18s, transform 0.15s; }
  .au-toolbtn:hover { transform: translateY(-1px); }

  .au-send:not(:disabled):hover { transform: translateY(-1px) scale(1.03); }
  ...
  @media (prefers-reduced-motion: reduce) {
    .au-core-ring, .au-core-dot, .au-aurora span, .meter-bar, .type-dot, .au-dot, .au-persona-btn { animation: none !important; }
  }
`
...
/* ─────────────────────────────────────────────
   MINI COMPONENTS
───────────────────────────────────────────── */
const Meter = ({ active, color = 'var(--au-cyan)', count = 4, h = 12 }) => (
...
/* Signature element: THE CORE — a single spinning ring around a solid
   dot. Quiet at rest, one clean gesture when listening / thinking /
   speaking, rather than a cluster of bars competing for attention. */
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox="0 0 46 46">
        ...
        <circle
          className={`au-core-ring ${active ? 'is-active' : ''} ${state === 'speaking' ? 'is-speaking' : ''}`}
          cx="23" cy="23" r="17" fill="none"
          stroke={active ? 'url(#au-core-grad)' : 'rgba(245,243,251,0.35)'}
          strokeWidth="1.4" strokeDasharray="4 3" opacity={active ? 0.9 : 0.5}
        />
        <circle className={`au-core-dot ${active ? 'is-active' : ''}`} cx="23" cy="23" r="4" fill={active ? 'url(#au-core-grad)' : 'rgba(245,243,251,0.5)'} />
      </svg>
    </div>
  )
}
...
const Avatar = ({ isUser, auraState }) => {
  if (!isUser) return <Core state={auraState} size={22} />
  return (
    <div className="w-[22px] h-[22px] rounded-full flex-shrink-0 flex items-center justify-center"
      ...
      <FiUser size={11} color="#fff" />
    </div>
  )
}

/* Persona switcher — simple pill row. The active pill scales up 2%
   and swaps to the gradient fill so the switch itself feels like an
   action, not just a color change. */
const PersonaRow = ({ persona, onSwitch }) => (
  <div className="au-tight au-persona-row px-4 py-2.5 border-b flex items-center gap-1.5 overflow-x-auto relative z-10"
    style={{ borderColor: 'var(--au-border)', background: 'rgba(10,11,20,0.4)' }}
  >
    {Object.entries(PERSONAS).map(([key, p]) => {
      const on = persona === key
      return (
        <button key={key} onClick={() => onSwitch(key)}
          className={`au-persona-btn px-2.5 py-1 rounded-full border text-[10px] whitespace-nowrap flex-shrink-0 ${on ? 'is-active' : ''}`}
          style={{
            background: on ? 'linear-gradient(90deg, rgba(94,234,212,0.20), rgba(167,139,250,0.20))' : 'var(--au-glass)',
            borderColor: on ? 'var(--au-border-bright)' : 'var(--au-border)',
            color: on ? 'var(--au-ink)' : 'var(--au-dim)',
        ...
        {!isOpen && (
          <motion.button
            ...
            style={{
              ...
              boxShadow: '0 14px 34px rgba(0,0,0,0.6), 0 0 0 1px rgba(167,139,250,0.10), 0 0 24px rgba(167,139,250,0.18)',
            }}
          >
            <motion.span
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="au-panel fixed z-[9998] w-[calc(100vw-1.5rem)] sm:w-[calc(100vw-2rem)] max-w-sm sm:max-w-md rounded-2xl flex flex-col relative"
            style={{
              bottom: 'max(0.75rem, env(safe-area-inset-bottom))',
              right: 'max(0.75rem, env(safe-area-inset-right))',
              maxHeight: 'min(620px, calc(100dvh - 1.5rem))',
          ...
          >
            ...
            {/* ── HEADER ── */}
            <div className="au-tight flex items-center gap-3 px-4 py-3.5 border-b relative z-10 sticky top-0"
              style={{ background: 'rgba(10,11,20,0.75)', backdropFilter: 'blur(18px)', borderColor: 'var(--au-border)' }}
            >
              <div className="relative flex-shrink-0 w-[38px] h-[38px] flex items-center justify-center rounded-full"
                style={{ background: 'var(--au-glass)', border: '1px solid var(--au-border-bright)' }}
              >
                <Core state={auraState} size={30} />
              </div>

              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 font-semibold text-sm min-w-0" style={{ color: 'var(--au-ink)', fontFamily: 'var(--au-font-display)', letterSpacing: '-0.01em' }}>
                  <span className="truncate">Nova</span>
                  <span className="hidden sm:inline-flex flex-shrink-0 text-[9px] font-medium tracking-wider px-2 py-[3px] rounded-full border uppercase"
                    style={{ color: 'var(--au-cyan)', borderColor: 'var(--au-border-bright)', fontFamily: 'var(--au-font-mono)' }}
                  >
                    Fullstack AI
                  </span>
                </div>
            ...
            {/* ── PERSONA SWITCHER ── */}
            <PersonaRow persona={persona} onSwitch={setPersona} />
            ...
            <AnimatePresence initial={false}>
              {!isMinimized && (
                ...
                >
                  {/* ── MESSAGES ── */}
                  <div className="au-tight au-messages h-[min(40dvh,240px)] sm:h-[260px] overflow-y-auto px-4 py-3.5 flex flex-col gap-2.5 relative z-10"
                  ...
                  >
                    {conversation.length === 0 && (
                      ...
                      >
                        ...
                        <p className="text-xs mt-1.5" style={{ color: 'var(--au-faint)', fontFamily: 'var(--au-font-mono)' }}>
                          {PERSONAS[persona].tagline?.toLowerCase() || 'fullstack · kigali, rwanda'}
                        </p>
                      </motion.div>
                    )}
                    ...
                    {conversation.map((msg, idx) => (
                      ...
                      >
                        <div className={`flex gap-2 max-w-[85%] ${msg.type === 'user' ? 'flex-row-reverse' : 'flex-row'} items-end`}>
                  ...
                  <AnimatePresence>
                    {transcript && isListening && (
                      <motion.div
                        ...
                        className="px-4 py-2 border-t flex items-center gap-2 overflow-hidden"
                  ...
                  {/* ── INPUT AREA ── */}
                  <div className="au-tight px-4 py-3 border-t" style={{ background: 'rgba(10,11,20,0.55)', borderColor: 'var(--au-border)' }}>
                    <div className="flex gap-2 items-center">
                      <button
                        className={`au-mic-btn w-9 h-9 rounded-full border flex items-center justify-center flex-shrink-0 ${isListening ? 'is-listening' : ''}`}
                      ...
                      >
                        {isListening ? (
                          <FaStop size={12} color="var(--au-pink)" />
                        ) : (
                          <Meter active={false} count={3} h={12} />
                        )}
                      </button>

                      <input
                        ref={inputRef}
                        className="au-input flex-1 min-w-0 h-9 px-4 rounded-full border"
                        type="text"
                        value={inputText}
                      ...
                      <motion.button
                        ...
                        className="au-send w-9 h-9 rounded-full border flex items-center justify-center flex-shrink-0"
                        style={{
                      ...
                      >
                        <FaPaperPlane size={12} color={inputText.trim() ? '#0a0c18' : 'var(--au-faint)'} />
                      </motion.button>
                    </div>
                  ...
                  <AnimatePresence>
                    {showSuggestions && (
                      ...
                      >
                        <div className="au-tight px-4 py-3">
